...
const Upcomming = () => {
    ...
    const {
        populars,
        status: popularsStatus,
        error: popularsError,
    } = useSelector(selectPopularMovieData);

    const {
        upcommings,
        status: upcomingsStatus,
        error: upcomingsError,
    } = useSelector(selectUcommingMovieData);

    useEffect(() => {
        if (popularsStatus === 'idle') {
            dispatch(fetchPopulars());
        }
        if (upcomingsStatus === 'idle') {
            // Added a condition for fetching upcoming movies
            dispatch(fetchUpcommings());
        }
    }, [popularsStatus, upcomingsStatus, dispatch]);
    ...
    return (
        <>
            {popularsStatus === 'loading' || upcomingsStatus === 'loading' ? (
                <div>Loading...</div>
            ) : popularsStatus === 'failed' || upcomingsStatus === 'failed' ? (
                <div>Error: {popularsError || upcomingsError}</div>
            ) : (
